...
public class RssFeedScheduler {
    ...
    public void resetQueueAndStartProcessing() {
        log.debug("RSS 피드 작업을 위해 대기열을 초기화합니다.");
    ...
    public void readRssFeeds() {
        if (!isQueueReset) {
            log.debug("Queue가 초기화되지 않았으므로 작업을 실행하지 않습니다.");
            return;  // 큐가 초기화되지 않은 경우 작업 중지
        }
        
        if (blogQueue.isEmpty()) {
            log.debug("모든 블로그의 RSS 피드를 읽었으므로, Queue가 초기화될 때까지 작업을 중지합니다.");
        ...
        if (blog != null) {
            try {
                ...
                log.debug("RSS 피드를 성공적으로 읽었습니다. Blog 명: {}", blog.getEnglishName());
            } catch (Exception e) {
                log.debug("RSS 피드를 읽는데 실패했습니다. Blog 명: {}", blog.getEnglishName(), e);
            }
        }
    }
    ...
    private void resetQueue() {
        ...
        log.debug("큐가 초기화되었습니다. {}개의 블로그가 대기열에 추가되었습니다.", blogQueue.size());
    }
}
